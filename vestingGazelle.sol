...
contract GazelleVesting{
    address public constant gazelleAddress = 0x1Fa36EF07d1a90624880f1f03Df18196c3d17f6C;
    IGazelle gazelle = IGazelle(gazelleAddress);
    uint256 decimalFactor = 10**uint256(gazelle.decimals());
    ...
    constructor(){
        ...
        vestingtTypesToDetails[vestingTypes[0]] = VestingDetails(vestingTypes[0], block.timestamp, block.timestamp + 600, block.timestamp + 1460 days, 77_000_000 * decimalFactor, 0, 0, 0, vestingtTypesToDetails[vestingTypes[0]].assignedAddresses);
        ...
        vestingtTypesToDetails[vestingTypes[2]] = VestingDetails(vestingTypes[2], block.timestamp, block.timestamp + 1 days, block.timestamp + 180 days, 50_000_000 * decimalFactor, 0, 0, 0, vestingtTypesToDetails[vestingTypes[2]].assignedAddresses);
    ...
    function mintForInvestorAndAirDrop() public onlyOwner{
        ...
        gazelle.mint(address(this), mintAmount_investor);
        ...
        gazelle.mint(address(this), mintAmount_airdrop);
    ...
    function mintTeamFund(uint256 mintAmount) public onlyOwner returns(bool){
        ...
        if(currentTime >= endTime){
            gazelle.mint(address(this), mintAmount);
    ...
    function transferFundsToAddress(uint256 indexOfVestingType, address assignedAddress, uint256 amount) public onlyOwner validAddress(assignedAddress) validIndex(indexOfVestingType) returns(bool){
        ...
        gazelle.transfer(assignedAddress, amount);
